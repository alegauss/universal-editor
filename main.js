import { copyExports as r } from "./copyExports.js";
import { connectToParent } from "./penpal.js";
import { defineProperty as o } from "./defineProperty.js";
import { ZodError as F } from "./ZodError.js";
import { addIssueToContext as K } from "./addIssueToContext.js";
import { ParseStatus as W } from "./ParseStatus.js";
import {writePrivateMember as re, readPrivateMember as ne} from "./utils";
import { ParseContext as oe } from "./ParseContext.js";
import { SchemaBase as ae } from "./SchemaBase.js";
  ...
  const createStore = (createState) => {
    ...
    const setState = (partial, replace) => {
        ...
        if (!Object.is(newState, oldState)) {
            state = (replace !== undefined ? replace : (typeof newState !== 'object' || newState === null))
                ? newState
                : { ...state, ...newState };
    ...
    const initialState = (state = createState(setState, getState, store));
    return store;
};

const createStoreWrapper = (createState) => {
    return createState ? createStore(createState) : createStore;
}

var wu = {};
(wu.createStore = createStoreWrapper),
(wu.default = createStoreWrapper),
((wu = createStoreWrapper).createStore = createStoreWrapper),
(wu.default = wu);
  let Eu = ({ set: e }, t) => {
      e((e) => ({ ...e, editor: t }));
    },
...
async function main() {
    let e = connectToParent({ methods: zu }),
      t = await e.promise;
